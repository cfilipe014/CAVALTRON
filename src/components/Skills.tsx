  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("skills").select("*").order("display_order", {
        ascending: true
      });
  ...
  return <section id="segmentos" className="py-20 md:py-32 bg-muted/30" aria-labelledby="skills-heading">
      <div className="container px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
          ...
          return <Card key={skill.id} className="border-border hover:border-accent transition-all duration-300 hover:shadow-[var(--shadow-card)] bg-card group">
                <CardHeader>
